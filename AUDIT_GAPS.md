...
| G-BLOC-009a | P2 | Financial correctness | Repartizare rounding penny-leak: per-apartment shares rounded independently → sum ≠ expense total | ✅ Eliminated 2026-06-24 (`7e17b5e`, deployed) | New pure `allocateByLargestRemainder` (Hamilton) helper in `src/lib/repartizare.ts`, computed over the FULL apartment set (subset-regen safe per G-BLOC-006). Wired into `calcul-chitanta.ts` + `avizier/export` proportional modes (COTA_INDIVIZA/PERSOANE/APARTAMENT). Shares now sum EXACTLY to the expense total. CONSUM + avizier historical restanță/penalty untouched. 9 jest specs incl. 500-case exact-sum property test. Build/jest green, live blocx.ro 200. |
| G-BLOC-009b | P2 | Financial correctness | Monetary fields `Float` not `Decimal` (accounting-rigor; precision drift on large sums) | OPEN — DEFERRED | XL refactor: 22 money fields, prod `ALTER COLUMN` migration, ~13 JSON routes (Decimal serializes as string), 30+ frontend sites, Decimal.js arithmetic rewrite. ~2-week dedicated multi-session track; not bundled with live NO-TOUCH deploy. Blast-radius map produced 2026-06-24. |
